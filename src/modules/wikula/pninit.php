 ...
 */
function wikula_init()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    // Main table
    if (!DBUtil::createTable('wikula_pages')) {
        return LogUtil::registerError(__('Error! Table creation failed.', $dom).' (wikula_pages)');
    }
    ...
    if (!DBUtil::createIndex('tag', 'wikula_pages', 'tag', $idxoptarray)) {
        return LogUtil::registerError(__('Error! Index creation failed.', $dom).' - tag (wikula_pages)');
    }
    ...
    //    return LogUtil::registerError(__('Error! Index creation failed.', $dom).' - body (wikula_pages)');
    //}


    // Links table
    if (!DBUtil::createTable('wikula_links')) {
        return LogUtil::registerError(__('Error! Table creation failed.', $dom).' (wikula_links)');
    }
    ...
    if (!DBUtil::createIndex('idx_from,idx_to', 'wikula_links', array('from_tag','to_tag'), $idxoptarray)) {
        return LogUtil::registerError(__('Error! Index creation failed.', $dom).' - idx_from,idx_to (wikula_links)');
    }


    // Referrers table
    if (!DBUtil::createTable('wikula_referrers')) {
        return LogUtil::registerError(__('Error! Table creation failed.', $dom).' (wikula_referrers)');
    }
    ...
    if (!DBUtil::createIndex('idx_page_tag,idx_time', 'wikula_referrers', array('page_tag','time'), $idxoptarray)) {
        return LogUtil::registerError(__('Error! Index creation failed.', $dom).' - idx_page_tag,idx_time (wikula_referrers)');
    }
    ...
    if ($wikulainit['importwikka']) {
        // If we do import
        if (!wikula_ImportData()) {
            return LogUtil::registerError(__('PROBLEM IMPORTING DATA', $dom));
        }
    // Filling with default data...
    } elseif (!wikula_DefaultData($wikulainit['root_page'])) {
        return LogUtil::registerError(__('PROBLEM INSERT DEFAULT DATA', $dom));
    }
    ...
    // Set the choosen Admin Panel category
    pnModAPIFunc('Admin', 'admin', 'addmodtocategory',
                 array('module'   => 'wikula',
                       'category' => $wikulainit['admincategory']));
 ...
 */
function wikula_upgrade($oldversion)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    switch($oldversion) {
        ...
        case '1.0':
            ...
            $sqls[] = "UPDATE $tables[wikula_pages] SET $column[tag] = '".__('WikiHelp', $dom)."' WHERE $column[tag] = 'WikkaDocumentation'";
            $sqls[] = "UPDATE $tables[wikula_pages] SET $column[tag] = '".__('ReleaseNotes', $dom)."' WHERE $column[tag] = 'WikkaReleaseNotes'";
            foreach ($sqls as $sql) {
            ...
            pnModSetVars('wikula', $wikulavars);
            pnModDelVar('pnWikka');
 ...
 */
function wikula_delete()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    // Delete the module tables
    if (!DBUtil::dropTable('wikula_pages')) {
        return LogUtil::registerError(__('Error! Table deletion failed.', $dom).' (wikula_pages)');
    }

    if (!DBUtil::dropTable('wikula_links')) {
        return LogUtil::registerError(__('Error! Table deletion failed.', $dom).' (wikula_links)');
    }

    if (!DBUtil::dropTable('wikula_referrers')) {
        return LogUtil::registerError(__('Error! Table deletion failed.', $dom).' (wikula_referrers)');
    }
 ...
 */
function wikula_init_interactiveinit()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    ...
    return $render->fetch('wikula_init_interactive.tpl');
}

function wikula_init_step2()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!SecurityUtil::checkPermission('::', '::', ACCESS_ADMIN)) {
        return LogUtil::registerError(__('Sorry! No authorization to access this module.', $dom), 403);
    ...
    if ($submit && empty($wikulainit['root_page'])) {
        $validationerror = __('Must fill the root Page', $dom);
    }

    // defaults
    $wikulainit['root_page']    = (!empty($wikulainit['root_page'])) ? $wikulainit['root_page'] : __('HomePage', $dom);
    ...
    if (!$submit || $validationerror !== false) {
        ...
        return $render->fetch('wikula_init_step2.tpl');

    } else {
        // submit is set, read the data and store them.
        if (!SecurityUtil::confirmAuthKey()) {
            return LogUtil::registerError(__("Invalid 'authkey':  this probably means that you pressed the 'Back' button, or that the page 'authkey' expired. Please refresh the page and try again.", $dom), null, pnModURL('Modules', 'admin', 'view'));
        }
    ...
    return pnRedirect(pnModURL('wikula', 'init', 'step3'));
}

function wikula_init_step3()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!SecurityUtil::checkPermission('::', '::', ACCESS_ADMIN)) {
        return LogUtil::registerError(__('Sorry! No authorization to access this module.', $dom), 403);
    ...
    $category = pnModAPIFunc('Admin', 'admin', 'get',
                             array('cid' => $wikulainit['admincategory']));

    $render = pnRender::getInstance('wikula', false);

    $render->assign($wikulainit);
    $render->assign('admincategory', $category['catname']);

    return $render->fetch('wikula_init_step3.tpl');
}

function wikula_init_interactivedelete()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    // Check permissions
    if (!SecurityUtil::checkPermission('::', '::', ACCESS_ADMIN)) {
    ...
    $render = pnRender::getInstance('wikula', false);
    return $render->fetch('wikula_init_delete.tpl');
}
 ...
 * Default Data
 */
function wikula_DefaultData($root_page=null)
{
    $dom = ZLanguage::getModuleDomain('wikula');
...
Enjoy!", $dom),
    ...
    if (!DBUtil::insertObject($record, 'wikula_pages')) {
        return LogUtil::registerError(__('Failed filling Database: ', $dom).' homepage');
    }
...
----
[[CategoryWiki Wiki category]]', $dom),
...
----
{{Category compact="1" notitle="1"}}', $dom),
...
[[CategoryWiki Wiki category]]', $dom));
    ...
    foreach ($tags as $name => $tag) {
        ...
        if (!DBUtil::insertObject($record, 'wikula_pages')) {
            return LogUtil::registerError(__('Failed filling Database: ', $dom).' page '.$name+1);
        }
    }
...
*/
   ...
   /* if (false == $result) {
        return LogUtil::registerError(__('Import data failed: ', $dom));
    }*/
...
/*
    ...
    for(; !$result->EOF; $result->MoveNext()) {
        ...
        if ($dbconn->ErrorNo() != 0) {
            return LogUtil::registerError(__('Import data failed: ', $dom).' target - '.$dbconn->ErrorMsg());
        }
    }
...
/*
    ...
    if ($dbconn->ErrorNo() != 0) {
        return LogUtil::registerError(__('PROBLEM IMPORTING DATA', $dom).' - '.$dbconn->ErrorMsg());
    }
