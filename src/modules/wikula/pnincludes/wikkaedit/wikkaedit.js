...
WikkaEdit.prototype.init = function() {
	// load CSS
	document.write("<link rel=\"stylesheet\" type=\"text/css\" href=\""+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/wikkaedit.css\" />");
...
WikkaEdit.prototype.genToolbar = function() {
	...
	for(var i in this.we_buttons) {
		switch (this.we_buttons[i].type) {
			case "button" :		// button
				html += "<img class=\"toolbutton\" src=\""+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/images/" + this.refToName(i) + ".gif\" alt=\"" + this.refToName(i) + "\" title=\"" + this.we_buttons[i].title + "\" onclick=\"varWikkaEdit.toolbarButtonClick(this, '" + this.refToName(i) + "');\"" + ie6_hover + " />";
				break;
			case "submenu" :	// submenu
				html += "<img class=\"toolbutton\" src=\""+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/images/submenu.gif\" alt=\"submenu\" title=\"" + this.we_buttons[i].title + "\" onclick=\"varWikkaEdit.toolbarButtonClick(this, '" + this.refToName(i) + "');\"" + ie6_hover + " />";
				break;
			case "separator" :	// separator
				html += "<img class=\"toolseparator\" src=\""+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/images/separator.gif\" alt=\"|\" />";
				break;
			default :
	...
	if (this.we_actionsMenuEnabled) {
		...
		for(i in this.we_categs) {
			if (this.we_categs[i].title != null)	// hidden actions don't have a title
				html += "<span class=\"toolbutton\" style='margin-left:20px; padding:2px 5px 3px 6px' onclick=\"varWikkaEdit.toolbarCategClick('" + i + "', this);\">" + this.we_categs[i].title + "<img src='"+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/images/submenu.gif' style='vertical-align:middle'/></span>";
		}
	}
	...
	if (newSubmenu == "shortcuts") {
		...
		if (!isAction) {
			for(i in this.we_buttons[this.nameToRef(newSubmenu)].we_buttons) {
				...
				html += "<img class=\"smimage\" src=\""+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/images/" + this.refToName(i) + ".gif\" alt=\"" + this.refToName(i) + "\"" + ie6_hover + " />";
			...
			for(i in this.we_actions) {
				if (this.we_actions[i].we_categ == newSubmenu) {
					...
					html += "<img class=\"smimage\" src=\""+Wikkaedit_baseURL+"modules/wikula/pnincludes/wikkaedit/images/actions/" + this.we_actions[i].we_name + ".gif\" alt=\"" + this.we_actions[i].we_name + "\"" + ie6_hover + " />";
...
// ===== run wikkaedit =====
var varWikkaEdit = new WikkaEdit(document.getElementById("wikula_body"));
