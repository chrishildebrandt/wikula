 ...
 * @version    $Id: pnuserapi.php 170 2010-05-02 12:10:46Z gilles $
 ...
 * Validate a PageName
 */
function wikula_userapi_isValidPagename($args)
{
    if (!isset($args['tag'])) {
 ...
 */
function wikula_userapi_isAllowedToEdit($args)
{
 ...
 */
function wikula_userapi_LoadPage($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['tag'])) {
        return LogUtil::registerArgsError();
    ...
    if (!SecurityUtil::checkPermission('wikula::', 'page::'.$args['tag'], ACCESS_READ)) {
        return LogUtil::registerError(__('You do not have the authorization to read this page!', $dom), 403);
    }
 ...
 */
function wikula_userapi_PageExists($args)
{
    if (!isset($args['tag'])) {
        return LogUtil::registerArgsError();
    }
...
function wikula_userapi_LoadPagebyId($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if ($page === false) {
        return LogUtil::registerError(__('Error! Getting the this page by id failed.', $dom));
    }
 ...
 */
function wikula_userapi_LoadRevisions($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['tag'])) {
        return LogUtil::registerArgsError();
...
function wikula_userapi_LoadPagesLinkingTo($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if ($links === false) {
        return LogUtil::registerError(__('Error! Getting the links for this page failed.', $dom));
    }
...
function wikula_userapi_CountBackLinks($args = array())
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['tag'])) {
        return LogUtil::registerArgsError();
    ...
    if ($links === false) {
        return LogUtil::registerError(__('Error! Count the links for this page failed.', $dom));
    }
 ...
 */
function wikula_userapi_LoadRecentlyChanged($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if ($revisions === false) {
        return LogUtil::registerError(__('Error! Getting revisions failed.', $dom));
    }
...
function wikula_userapi_LoadAllPages($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if ($pages === false) {
        return LogUtil::registerError(__('Error! Getting all pages failed.', $dom));
    }
...
function wikula_userapi_LoadAllPagesEditedByUser($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if ($pages === false) {
        return LogUtil::registerError(__('Error! Getting all pages by user failed.', $dom));
    }
 ...
 */
function wikula_userapi_CountAllPages()
{
    $dom = ZLanguage::getModuleDomain('wikula');
    // build the where clause
    $pntable =& pnDBGetTables();
 ...
 */
function wikula_userapi_LoadAllPagesOwnedByUser($args)
{
    ...
    $result['total'] = pnModAPIFunc('wikula', 'user', 'CountAllPages');

    return $result;
}

function wikula_userapi_FullTextSearch($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    $sql = 'SELECT '.$col['id'].', '
                ...
                .' FROM '.$tbl
                .' WHERE '.$col['latest'].'="Y" '
                .' AND (MATCH('.$col['tag'].') AGAINST("'.$phrase.'") '
                .' OR MATCH('.$col['body'].') AGAINST("'.$phrase.'"'.$boolean.')) '
    ...
    if ($dbconn->ErrorNo() != 0) {
        return LogUtil::registerError(__('Getting matches for this page failed!', $dom).' (1) - '.$dbconn->ErrorMsg());
    }
    ...
    if ($result->EOF) {
        ...
        if ($dbconn->ErrorNo() != 0) {
            return LogUtil::registerError(__('Getting matches for this page failed!', $dom).' (2) - '.$dbconn->ErrorMsg());
        }
    }
...
function wikula_userapi_LoadWantedPages($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    $pagetbl = &$pntable['wikula_pages'];
    $pagecol = &$pntable['wikula_pages_column'];
    $linktbl = &$pntable['wikula_links'];
    $linkcol = &$pntable['wikula_links_column'];
    ...
    if ($dbconn->ErrorNo() != 0) {
        return LogUtil::registerError(__('Getting matches for this page failed!', $dom).' - '.$dbconn->ErrorMsg());
    }

    if ($result->EOF) {
        return LogUtil::registerError(__('No items found.', $dom));
    }
    $pages = array();
...
function wikula_userapi_LoadOrphanedPages($args)
{
...
function wikula_userapi_IsOrphanedPage($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    $pagecol = &$pntable['wikula_pages_column'];
    $linktbl = &$pntable['wikula_links'];
    $linkcol = &$pntable['wikula_links_colum'];
...
function wikula_userapi_DeleteOrphanedPage($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if (!SecurityUtil::checkPermission('wikula::', 'page::'.$tag, ACCESS_DELETE))  {
        return LogUtil::registerError(__('You do not have the right to delete this page!', $dom), 403);
    }
    ...
    if (!DBUtil::deleteWhere('wikula_pages', $pagewhere)) {
        return LogUtil::registerError(__('Error! Deleting this page failed.', $dom));
    }

    $linkcol   =& $pntable['wikula_links_colum'];
    $linkwhere = 'WHERE '.$linkcol['from_tag'].' = "'.DataUtil::formatForStore($tag).'"';
    if (!DBUtil::deleteWhere('wikula_links', $linkwhere)) {
        return LogUtil::registerError(__('Error! Deleting links for this page failed.', $dom));
    }

    $aclscol   =& $pntable['wikula_acls_column'];
    $aclswhere = 'WHERE '.$aclscol['page_tag'].' = "'.DataUtil::formatForStore($tag).'"';
    if (!DBUtil::deleteWhere('wikula_acls', $aclswhere)) {
        return LogUtil::registerError(__('Error! Deleting ACLS for this page failed.', $dom));
    }

    $refscol   =& $pntable['wikula_referrers_colum'];
    $refswhere = 'WHERE '.$refscol['page_tag'].' = "'.DataUtil::formatForStore($tag).'"';
    if (!DBUtil::deleteWhere('wikula_referrers', $refswhere)) {
        return LogUtil::registerError(__('Error! Deleting referers for this page failed.', $dom));
    }

    return LogUtil::registerStatus(__('Done! Page deleted with success.', $dom));
}

function wikula_userapi_SavePage($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if (!SecurityUtil::checkPermission('wikula::', 'page::'.$tag, ACCESS_COMMENT))  {
        return LogUtil::registerError(__('You do not have the authorization to add a page!', $dom), 403);
    }
    ...
    $oldpage = pnModAPIFunc('wikula', 'user', 'LoadPage', array('tag' => $tag));

    // only save if new body differs from old body
    if ($oldpage && $oldpage['body'] == $body) {
        return LogUtil::registerError(__('The content of the page to save is the same of the current revision. New page not saved.', $dom));
    }
    ...
    if ($res === false) {
        return LogUtil::registerError(__('Setting last revision failed!', $dom));
    }
    ...
    if ($res === false) {
        return LogUtil::registerError(__('Saving revision failed!', $dom));
    }
/*
    if ((isset($args['tracking']) && $args['tracking'] ) || SessionUtil::getVar('tracking')) {
        SessionUtil::setVar('wikula_previous', $tag);
        pnModAPIFunc('wikula', 'user', 'wakka', array('text' => $body));
    }
*/
    // TODO: Wikka Ping feature here

    if (!$oldpage) {
        LogUtil::registerStatus(__('New page created!', $dom));
    } else {
        LogUtil::registerStatus(__('New revision saved!', $dom));
    }
...
function wikula_userapi_PurgePages($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    ...
    $days = pnModGetVar('wikula', 'pages_purge_time');
...
// INTERWIKI STUFF
function wikula_userapi_ReadInterWikiConfig()
{
    $dom = ZLanguage::getModuleDomain('wikula');
...
function wikula_userapi_AddInterWiki($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if (!isset($wikiname) || !isset($wikiurl)) {
        return LogUtil::registerError(__('Adding InterWiki failed due to missing arguments!', $dom));
    }

    $interwiki = unserialize(pnModGetVar('wikula', 'interwiki'));
    ...
    if (!pnModSetVar('wikula', 'interwiki', serialize($interwiki))) {
        return LogUtil::registerError(__('Adding interwiki failed!', $dom));
    }

    return LogUtil::registerStatus(__('Interwiki added with success!', $dom));
}

function wikula_userapi_GetInterWikiUrl($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if (!isset($name) || !isset($tag)) {
        return LogUtil::registerError(__('Error! Invalid arguments.', $dom));
    }

    $interwiki = pnModAPIFunc('wikula', 'user', 'ReadInterWikiConfig');
 ...
 */
function wikula_userapi_Link($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['tag'])) {
        return false;
    ...
    if (preg_match('/^([A-Z][A-Z,a-z]+)[:]([A-Z,a-z,0-9]*)$/s', $args['tag'], $matches)) {
        ...
        $link = pnModAPIFunc('wikula', 'user', 'GetInterWikiUrl',
        ...
        // it's a Wiki link!
        $pageid = pnModAPIFunc('wikula', 'user', 'PageExists',
                               array('tag' => $args['tag']));
        ...
        if (!empty($pageid)) {
            //$pnurl = urlencode(pnModUrl('wikula', 'user', 'main', array('tag' => $args['tag'])));
            //$text = DataUtil::formatForDisplay($args['text']);
            return '<a href="'.pnModUrl('wikula', 'user', 'main', array('tag' => DataUtil::formatForDisplay(urlencode($args['tag'])))).'" title="'.$args['text'].'">'.$args['text'].'</a>';
        } else {
            return '<span class="missingpage">'.$args['text'].'</span><a href="'.pnModUrl('wikula', 'user', 'edit', array('tag' => urlencode($args['tag']))).'" title="'.DataUtil::formatForDisplay($args['tag']).'">?</a>';
        }
    }
...
function wikula_userapi_TrackLinkTo($tag)
{
    $linktable = SessionUtil::getVar('linktable');
...
function wikula_userapi_GetLinkTable()
{
    return unserialize(SessionUtil::getVar('linktable'));
}

function wikula_userapi_ClearLinkTable()
{
    return SessionUtil::delVar('linktable');
}

function wikula_userapi_StartLinkTracking()
{
    SessionUtil::setVar('linktracking', 1);
}

function wikula_userapi_StopLinkTracking()
{
    SessionUtil::setVar('linktracking', 0);
}

function wikula_userapi_WriteLinkTable($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['tag'])) {
        return LogUtil::registerArgsError();
    ...
    if (is_array($linktable)) {
        ...
        foreach ($linktable as $to_tag) {
            ...
            if (!isset($written[$lower_to_tag])) {
                ...
                if ($record === false) {
                    return LogUtil::registerError(__('Inserting link failed!', $dom));
                }
 ...
 */
function wikula_userapi_LogReferer($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    ...
    if ($referrer && !preg_match('/^'.preg_quote(pnGetBaseUrl(), '/').'/', $referrer)) {
        ...
        if ($writeref == true) {
            ...
            if ($res === false) {
                return LogUtil::registerError(__('Inserting Referer failed!', $dom));
            }
        }
...
function wikula_userapi_LoadReferrers($args)
{
    ...
    if ($result === false) {
        return LogUtil::registerError(__('Error! Get page failed.', $dom));
    }
...
function wikula_userapi_CountReferers($args = array())
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['tag'])) {
        return LogUtil::registerArgsError();
...
function wikula_userapi_ReturnSafeHTML($args)
{
    extract($args);
    unset($args);
...
function wikula_userapi_GeSHi_Highlight($args)
{
    extract($args);
    unset($args);
    ...
    $geshi_header = pnModGetVar('wikula', 'geshi_header');
    ...
    if ($start > 0) {
        ...
        $geshi_line_numbers = pnModGetVar('wikula', 'geshi_line_numbers', '1');
    ...
    $geshi_tab_width = pnModGetVar('wikula', 'geshi_tab_width', 4);
    if ($geshi_tab_width) {
...
function wikula_userapi_Action($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['action'])) {
        return LogUtil::registerError(__('Action argument missing!', $dom));
    }
    ...
    return pnModAPIFunc('wikula', 'action', strtolower($action), $vars);
}

function wikula_userapi_FullCategoryTextSearch($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    extract($args);
    unset($args);
    ...
    if ($result === false) {
        return LogUtil::registerError(__('Error! Happened during element fetching.', $dom));
    }
 ...
 */
function wikula_userapi_CheckMySQLVersion($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    $major    = $args['major'];
    $minor    = $args['minor'];
 ...
 */
function wikula_userapi_htmlspecialchars_ent($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    if (!isset($args['text']) || empty($args['text'])) {
        return '';
 ...
 */
function wikula_userapi_wakka($args)
{
    $dom = ZLanguage::getModuleDomain('wikula');
    global $mapcounter;
    $mapcounter = 1;
    ...
    if ($linktracking) {
       pnModAPIFunc('wikula', 'user', 'ClearLinkTable');
    }
    ...
    // close open tags
    $args['text'] .= wakka2callback('closetags');
...
function wakka2callback($things)
{
    ...
    // urls
        ...
        if (preg_match('/^(.*)\.(gif|jpg|jpeg|png)/si', $url)) {
            ...
            if (preg_match('/^(.*)\.(mm)/si', $url)) {
                ...
                return pnModAPIFunc('wikula', 'user', 'Link',
                                    array('tag'    => $url,
                                          'method' => '',
    ...
    // escaped text
        ...
        $ddquotes_policy = pnModGetVar('wikula', 'double_doublequote_html', 'safe');
        $embedded = $matches[1];
        ...
        if ($ddquotes_policy == 'safe' || $ddquotes_policy == 'raw')
        {
            ...
            for ($i=0; $i < $tags2; $i++)
            {
                ...
                if ($id != ($newid = wikula_userapi_makeId('embed', $id))) {
        ...
        switch ($ddquotes_policy)
        {
            ...
            default:
                return wikula_userapi_htmlspecialchars_ent(array('text' => $embedded)); // display only
        }
    ...
    // code text
        ...
        if (isset($language) && $geshi_path && file_exists("{$geshi_hi_path}/{$language}.php")) {
            ...
            $output = pnModAPIFunc('wikula', 'user', 'GeSHi_Highlight',
        ...
        if (pnModGetVar('wikula', 'grabcode_button', true)) {
            // build form
            $output .= '<form method="post" action="'.pnModURL('wikula', 'user', 'grabcode').'">
                        <input type="submit" name="save" class="grabcode" value="'.__('Grab code', $dom).'" title="'.__('Grab code', $dom).'" />
    ...
    // @@@ regex accepts NO non-whitespace before whitespace, surely not correct? [[  something]]
        ...
        if (isset($matches[1]) && !empty($matches[1])) {
            ...
            return $result.pnModAPIFunc('wikula', 'user', 'Link',
                                        array('tag'  => $url,
                                              'text' => $text));
        } else {
            return '';
    ...
    //} else if (preg_match("/^\!{0,1}[A-Z���]+[a-z����]+[A-Z0-9���][A-Za-z0-9�������]*$/s", $thing)) {
        ...
        if ($thing[0] == '!') {
            ...
            return pnModAPIFunc('wikula', 'user', 'Link', array('tag' => $thing));
        }
    ...
    // Removing this until it's been worked out
        ...
        return wikula_userapi_htmlspecialchars_ent(array('text' => $thing));
    }
...
function parse_attributes($attribs, $hints)
{
    ...
    foreach ($attribs as $attrib)
    {
        ...
        if (!$a) {
            ...
            $return_value .= ' '.$a.'="'.wikula_userapi_htmlspecialchars_ent(array('text' => $value)).'"';
        }
    }
...
function wakka3callback($things)
{
    ...
    if (preg_match(PATTERN_MATCH_HEADINGS, $thing, $matches))
    {
        ...
        if (preg_match(PATTERN_MATCH_ID_ATTRIBUTES, $h_attribs)) {
            ...
            $headingtext = CleanTextNode($h_heading);
            // now create id based on resulting heading text
            $h_id = wikula_userapi_makeId(array('group' => 'hn', 'id' => $headingtext));
...
function CleanTextNode($textvalue, $pattern_prohibited_chars = PATTERN_INVALID_ID_CHARS, $decode_html_entities = true)
{
    $dom = ZLanguage::getModuleDomain('wikula');
 ...
 * @return final id
 */
function wikula_userapi_makeId($args)
{
    if (!isset($args['group'])) {
        return LogUtil::registerArgsError();
    }
